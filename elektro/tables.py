...
* **A type the server will not be able to describe.** A float16 column reads back as ``FLOAT``
  ...
  Parquet to record at all. See :func:`elektro.vocabulary.duckdb_type`, which maps only what
...
from elektro.vocabulary import duckdb_type

if TYPE_CHECKING:
    import pyarrow as pa

    from elektro.api.schema import ColumnInput
...
def arrow_schema_of(frame: object) -> pa.Schema:
    ...
    This is the same conversion :func:`elektro.io.upload._parquet_payload`
    ...
    from elektro.scalars import ParquetLike
