"""Helpers for reading Elektro parquet objects out-of-core via DuckDB + httpfs.
...
if TYPE_CHECKING:
    ...
    from elektro.io.obstore import S3UploadGrantLike
...
def require_duckdb() -> "Any":  # noqa: ANN401
    ...
    dependency. Install it via ``elektro[table]``.
    """
    try:
        import duckdb
        ...
        raise ImportError(
            ...
            "Install it with `pip install elektro[table]`."
        ) from e
    return duckdb
...
def create_duckdb_s3_connection(
    ...
    """Create a DuckDB connection configured to read from the Elektro S3 endpoint.
