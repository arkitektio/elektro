...
`elektro` ships as a PyPI package (`elektro`). Versioning is automated by
...
| --- | --- | --- |
| `fix:` | patch | `fix: handle empty trace` |
| `feat:` | minor | `feat: add neuron recorder` |
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install elektro`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install elektro --pre` or an exact pin |
| `N.x` (e.g. `1.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain `pip install elektro`
...
```sh
pip install elektro --pre          # latest rc (or stable, whichever is newer)
pip install 'elektro==2.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install elektro`) are unaffected by the `next` channel.
