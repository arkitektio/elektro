"""elektro's structures: what it can send across a node boundary, and how.
...
from elektro import arkitekt as declared  # noqa: E402

#: The identifiers elektro puts on the wire. Changing one is a cross-service change,
#: so they are written out rather than derived from the thing under test.
IDENTIFIERS = {
    "@elektro/annotation",
    "@elektro/annotationcollection",
    "@elektro/arraydataset",
    "@elektro/coordinatesystem",
    "@elektro/dataset",
    "@elektro/experiment",
    "@elektro/file",
    "@elektro/lens",
...
def test_merging_stamps_the_service_that_owns_them() -> None:
    ...
    app = AppRegistry()
    app.merge(declared.registry, service="elektro")

    services = {s.service for s in app.structure_registry.structures()}
    assert services == {"elektro"}
